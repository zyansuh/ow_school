...
| 2 | **Discord 동기화 1회 실행** | `/admin` → 「Discord 동기화」 — 닉·역할·학생 수·반장 연결 일괄 정리 | 🔴 즉시 |
| 3 | **담당 학생 수 검증** | 반장 카드 인원 = 상세 페이지 인원 일치 여부 수동 확인 (2~3명 샘플) | 🔴 즉시 |
| 4 | **닉 변경 E2E 테스트** | 마이페이지 닉 변경 → 새로고침 → 로그아웃/재로그인 → 포인트·면담·반장 연결 유지 | 🔴 즉시 |
...
| # | 항목 | 현재 문제 | 권장 조치 |
|---|------|----------|----------|
| 6 | **`resolveTeacherForUser` 역할 폴백** | Discord 역할만 있으면 `findFirst()`로 아무 반장 반환 | 역할→반장 매핑 테이블 또는 Teacher 등록 시 `discordUserId` 필수 입력 |
| 7 | **반장 `discordUserId` 백필 UI** | username 매칭만으로는 불완전 | 관리자 반장 수정 폼에 Discord User ID 입력 필드 추가 |
...
| 14 | **로그인 시 자동 Teacher 링크** | 반장 계정 로그인 → `discordUserId` 자동 백필 + `isTeacher` 세션 갱신 |
| 15 | **관리자 권한 요청 플로우** | `AdminRoleRequest` 테이블 + `/admin/roles` 승인 UI (README 향후 설계 구현) |
| 16 | **반장 담당 학생 엑셀** | `/teacher/students` 또는 관리자에서 CSV/엑셀 다운로드 |
| 17 | **졸업면담 알림** | 제출 시 담당 반장 Discord DM 또는 채널 웹훅 |
| 18 | **포인트 사용처** | 포인트 조회만 가능 → 상점·리워드 연동 설계 |
| 19 | **신청 대기(pending) 플로우** | 현재 즉시 approved — 관리자 승인 대기 옵션 |
| 20 | **반 이동 / 담당 반장 변경** | 관리자 UI에서 `teacherId` 변경 시 기존 면담·포인트 유지 검증 |
...
| 27 | **감사 로그 확장** | 반장 변경·담당 학생 이동·포인트 수동 조정 기록 |
...
2. **반장 3명 수동 검증** — 카드 인원 / 상세 / teacher 페이지 일치  
3. **`next-themes` 제거** — 5분 작업, 의존성 정리  
4. **관리자 반장 폼에 `discordUserId`** — 재발 방지 핵심  
...
- [x] 학생↔반장 `teacherId` 기준 통일  
- [x] 담당 학생 수 live count + `syncTeacherStudentCount`  
- [x] 반장 상세 — 담당 학생 표 (서버닉·반·신청일·졸업·면담)  
- [x] 다크모드 토글 제거 (다크 고정)  
